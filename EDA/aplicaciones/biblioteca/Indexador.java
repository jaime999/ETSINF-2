...
public class Indexador {
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        ...
                        ListaConPI pi;
                        if(indices.recuperar(clave)==null){
                            pi = new LEGListaConPI();
                            pi.insertar(ind);
                            indices.insertar(clave,pi);
                        }
                        else{
                            pi = indices.recuperar(clave);
                            pi.fin();
                            pi.insertar(ind);
                            indices.insertar(clave,pi);
                        }
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        // COMPLETAR
        ListaConPI<Indice> pi = indices.recuperar(pal);
        res.inicio();
        pi.inicio();
        while(!pi.esFin()){                      
            String s = pi.recuperar().toString();            
            res.siguiente();
            pi.siguiente();
            int t = res.talla();
            int tt = pi.talla();
