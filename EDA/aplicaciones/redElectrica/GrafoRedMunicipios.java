...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        for(int i = 0; i<aRM.length; i++){
            //Devuelve su coste mínimo
            coste += aRM[i].getPeso();            
            ListaConPI<Municipio> lm = new LEGListaConPI<Municipio>();
            res = verticesAMunicipios.recuperar(aRM[i].getOrigen());            
            dest = verticesAMunicipios.recuperar(aRM[i].getDestino());
            lm.insertar(dest);
            ListaConPI<Municipio> lant = adyKruskal.insertar(res,lm);
            if(lant != null){
                lant.insertar(dest);
                adyKruskal.insertar(res,lant);
            }
        }        
