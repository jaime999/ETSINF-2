    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        ...
        if(ini <= fin){
            ...
            if(ini == fin){
                ...
                NodoABB<E> izq = construirEquilibrado(v,ini,mediana-1);
                NodoABB<E> der = construirEquilibrado(v,mediana+1,fin);
                res = new NodoABB<E>(v[mediana],izq,der);
                res.talla = 1 + talla(res.izq) + talla(res.der);
            }
        }
        return res;/*
        if(ini > fin ) return null;
        if(ini == fin) return new NodoABB<E>(v[ini]);
        int m = (ini + fin) / 2;
        NodoABB<E> res = new NodoABB<E>(v[m]);
        res.izq = construirEquilibrado(v,ini,m-1);
        res.der = construirEquilibrado(v,m+1,fin);
        res.talla = 1 + talla(res.izq) + talla(res.der);
        return res;*/
    }
