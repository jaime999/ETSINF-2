...
public class GrafoNoDirigido extends GrafoDirigido {
    ...
    public GrafoNoDirigido(int numV, Arista[] arcos) {
        /*super(numV);
        esDirigido = false;
        arbolRecubrimientoBFS();*/
        super(numV);numA = 0; esDirigido = false;
        /*Arista[] arc = arbolRecubrimientoBFS();
        arcos = arc;*/
        if(arcos != null){
            for (int i = 0; i < arcos.length;i++){
                elArray[arcos[i].getOrigen()].insertar(new Adyacente(arcos[i].getDestino(), arcos[i].getPeso())); 
                elArray[arcos[i].getDestino()].insertar(new Adyacente(arcos[i].getOrigen(), arcos[i].getPeso()));
                numA++; 
            }
        }
    }
