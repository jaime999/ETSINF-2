...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    protected final void rehashing() {
        TablaHash<C,V> auxiliar = new TablaHash((int)(elArray.length*2*FACTOR_CARGA));
        ListaConPI<C> claves = claves();
        for(claves.inicio(); !claves.esFin(); claves.siguiente()){
            auxiliar.insertar(claves.recuperar(), recuperar(claves.recuperar()));
        }
        this.elArray = auxiliar.elArray;
    } 
    ...
    public final double desviacionTipica() {
        ...
        double res = 0.0;
        for(int i =0; i<elArray.length; i++){
            res += Math.pow(elArray[i].talla()-factorCarga, 2);
        }
